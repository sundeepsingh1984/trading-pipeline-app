...
from app.models.alpaca_db_models import Stocks,BarMinute,BarHour,BarDaily
...
class BarController():
    ...
    async def get_bar_min_by_ticker(self,ticker,limit,start=None,end=None):

        data=await self.query_data(BarMinute,"prices_min",parameter_val = ticker,limit=limit,start=start,end=end)
        return data


    

    async def get_bar_hour_by_ticker(self,ticker,limit,start=None,end=None):

        data=await self.query_data(BarHour,"prices_hour",parameter_val = ticker,limit=limit,start=start,end=end)
        return data
    ...
    async def get_bar_daily_by_ticker(self,ticker,limit,start=None,end=None):

        data=await self.query_data(BarDaily,"prices_daily",parameter_val = ticker,limit = limit,start = start,end = end)
        return data
...
# price=asyncio.run(obj.get_bar_daily_by_ticker(["AMT", "BXP","RPT","PGRE","WPC"],10000))
# print(price)